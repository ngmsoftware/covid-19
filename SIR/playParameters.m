...
sldBeta = uislider(F,'Position',[20 35 300 3],'Limits',[19 21]);
sldGamma = uislider(F,'Position',[400 35 300 3],'Limits',[19 21]);
...
subplot(3,1,1);
hold('on');
plot(1:length(s),s(1,:),'r:','linewidth',4);
subplot(3,1,2);
hold('on');
plot(1:length(s),N-s(1,:)-s(2,:),'g:','linewidth',4);
subplot(3,1,3);
hold('on');
plot(1:length(s),s(2,:),'b:','linewidth',4);
